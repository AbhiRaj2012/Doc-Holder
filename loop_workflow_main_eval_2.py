...
# DeepEval is optional; the build loop works fine without it.
...
def reviewer_node(state: AgentState):
    ...
    if review_output.strip().upper() == "PASS":
...
# ==========================================
# 3B. DEEPEVAL EVALUATION (LLM-as-judge, optional)
# ==========================================
if DEEPEVAL_AVAILABLE:
    class OllamaEvalModel(DeepEvalBaseLLM):
        # Wraps the same local Ollama model so GEval runs fully offline.
...
def _build_deepeval_metrics() -> list:
    # Two G-Eval (LLM-as-judge) metrics, scored 0.0-1.0, reused across files.
    judge = OllamaEvalModel(llm, getattr(llm, "model", "ollama-model"))
    return [
        GEval(
            name="Correctness",
            criteria=(
                "The ACTUAL_OUTPUT is a generated code file. Using the requirements and manifest given in "
                "INPUT as ground truth: do element IDs/selectors used in the code match the manifest? Does "
                "each function's logic plausibly match its described purpose? Flag any obvious runtime bug."
            ),
        ...
        GEval(
            ...
            criteria=(
                ...
                "implement everything required, not left as a comment, TODO, or empty stub?"
            ),
            evaluation_params=[LLMTestCaseParams.INPUT, LLMTestCaseParams.ACTUAL_OUTPUT],
            model=judge,
            threshold=0.6,
...
def deepeval_score_file(code: str, requirements: str, manifest: str, metrics: list) -> Optional[dict]:
    # Scores one file's code against requirements + manifest context.
...
def evaluator_node(state: AgentState):
    print("\n📊 [Evaluator] Running DeepEval scoring...")
    metrics = _build_deepeval_metrics() if DEEPEVAL_AVAILABLE else []

    report = {}
    for filename, code in state["file_system"].items():
        report[filename] = {
            "chars": len(code),
            "deepeval": deepeval_score_file(code, state["requirements"], state["project_manifest"], metrics),
        }

    scores = [
        m["score"] for f in report.values() if f.get("deepeval")
        for m in f["deepeval"].values() if m.get("score") is not None
    ]
    overall = {
        "deepeval_available": DEEPEVAL_AVAILABLE,
        "deepeval_avg_score": round(sum(scores) / len(scores), 2) if scores else None,
    }

    (state["run_dir"] / "evaluation.json").write_text(
        json.dumps({"overall": overall, "files": report}, indent=2), encoding="utf-8"
    )
    write_log(state["run_dir"], "Evaluator", json.dumps(overall, indent=2))

    if overall["deepeval_avg_score"] is not None:
        print(f"   Avg DeepEval score: {overall['deepeval_avg_score']}/1.0")
    else:
        print("   DeepEval not installed — see evaluation.json for details.")
    return {}
...
def route_review(state: AgentState) -> str:
    return "pass" if state["review_attempts"] == "" else "fail"
...
if __name__ == "__main__":
    ...
    print(f"\n🎉 Build Complete! Check execution_log.txt and evaluation.json in {run_dir}")