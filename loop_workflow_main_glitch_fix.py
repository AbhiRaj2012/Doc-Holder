...
def reviewer_node(state: AgentState):
    ...
    # FIX 1: Catch empty LLM responses immediately
    if not review_output:
        print(f"   ⚠️ LLM returned an empty response. Forcing re-review. Review Count: {state['review_attempts']}")
        return {
            "feedback": "Empty response from reviewer. Please double check code against manifest requirements.",
            "review_attempts": state["review_attempts"] + 1,
            "review_status": "fail"
        }

    # FIX 2: Relax check to allow for markdown like "**PASS**"
...
def route_review(state: AgentState) -> str:
    # Safely get the status, default to "fail" to prevent accidental skips
    return state.get("review_status", "fail")
